...
#Command Error
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandNotFound):
            ...
            ba = discord.Embed()
            ba.add_field(name="<:aviError:777096756865269760> Bad Argument", value=error)
            await ctx.send(embed=ba, delete_after=10)
        
        elif isinstance(error, commands.BotMissingPermissions):
            bm = discord.Embed()
            bm.add_field(name="<:aviError:777096756865269760> I have no permission", value=f"I do not have permission to do that, Please give me these permission(s) `{missing_perms}`")
            ...
            ee = discord.Embed()
            ee.add_field(name="<:aviError:777096756865269760> Unknown Error", value="Uh oh, an error has occured. The error has been recorded and will be fixed soon.")
            try:
