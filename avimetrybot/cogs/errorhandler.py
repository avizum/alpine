...
#Command Error
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, Blacklisted):
            ...
            cd=discord.Embed(title="Slow down", description=f"This command is on cooldown. Try again in {humanize.naturaldelta(error.retry_after)}.", color=discord.Color.red())
            ...
            try:
                ...
                pass
            a = discord.Embed(title="Missing Arguments", description=f"You need to put the `{error.param.name}` parameter to run this command", color=discord.Color.red())
