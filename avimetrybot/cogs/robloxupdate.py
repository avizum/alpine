...
class RobloxUpdate(commands.Cog, name="roblox update"):
    ...
    async def update_check(self):
        async with self.avimetry.session.get("http://setup.roblox.com/version") as old_version:
            a = await old_version.text()
        await asyncio.sleep(10)
        async with self.avimetry.session.get("http://setup.roblox.com/version") as new_version:
        ...
        if b != a:
            ...
            embed.set_footer(text="If you want to get notified when ROBLOX updates, use the command 'a.updateping'.")
