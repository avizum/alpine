...
class Song:
    ...
    def create_embed(self):
        ...
        em.add_field(
            name='<:time:804063156259258368> Duration',
            value=f"`{self.source.duration}`",
            inline=True
        )
        em.add_field(
            name='<:user:804063696799662101> Requested by',
            value=self.requester.mention,
            inline=True
        )
        em.add_field(
            name='<:artist:804063511470800916> Artist',
            value='[{0.source.uploader}]({0.source.uploader_url})'.format(self)
        )
        em.add_field(
            name="<a:9655_eyesshaking_new:809789083127382066> Total Views",
            value="`{}`".format(humanize.intword(self.source.views))
        )
        em.add_field(
            name="<a:4985_thumbs_up:809789082276331550> Total Likes",
            value="`{}`".format(humanize.intword(self.source.likes))
        )
        em.add_field(
            name="<a:3505_thumps_down:809789081977356329> Total Dislikes",
...
class Music(commands.Cog):
    ...
    async def _stop(self, ctx):
        ...
        em = discord.Embed(title=":zzz: Alright, i'll stop the current song.", color=ctx.author.color)
